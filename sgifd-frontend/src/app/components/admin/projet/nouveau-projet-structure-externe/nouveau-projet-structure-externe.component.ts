...
@Component({
  selector: 'app-nouveau-projet-structure-externe',
  templateUrl: './nouveau-projet-structure-externe.component.html',
  styleUrls: ['./nouveau-projet-structure-externe.component.css']
})
export class NouveauProjetStructureExterneComponent implements OnInit {
  ...
  ngOnInit() {
    this.user = JSON.parse(this.tokenStorage.getCurrentUser());
    if(this.user == null || this.user.structureBeneficiaire == null) {
  ...
  returnBackTolIST(): void {
    ...
    this.router.navigate(['admin-structure-externe/projet-structure/list-projet-en-cours']);
  }
  ...
  makeFormFond(): void {
    ...
    this.validateFormProjetIdee_Etape1 = this.fb.group({
      ...
      niveaumaturite: [this.projetSubmit != null ? this.projetSubmit.niveaumaturite : null,
        [Validators.required]],
      structureSousTutelle: [this.user != null ? this.user.structureBeneficiaire : null],
